...
# モブが一定数以上存在する時、64ブロック以上離れており、飼い主がおらず、名前もつけられていないモブを削除する
...
execute if score #num_mobs_max RA <= #RA RA as @e[type=#random_all:all_mobs,tag=!RA_near_player] if entity @s[tag=RA_initialized] unless data entity @s Owner unless data entity @s CustomName run function random_all:kill_mobs
